...
EXPERTS = {
    ...
    "researcher": {
        ...
        "system": ("You are RECON, the RESEARCH expert for AgenticBuilderNews. You arm a scriptwriter with the "
                   ...
                   "NUMBERS: concrete figures that land — price, speed, size, benchmark wins, adoption, version. "
                   "If you don't KNOW a number, give a realistic ~approx and mark it; never invent a fake precise "
                   "number.\n"
    ...
    "scriptwriter": {
        ...
        "system": ("You are the SCRIPT expert for AgenticBuilderNews. Write ONE tight SPOKEN VO beat for a "
                   ...
                   "CONCRETE VARIETY (drives the on-screen visuals — DO NOT skip): within each beat, include AT "
                   "LEAST TWO of these THREE whenever the brief supports them: (a) a NUMBER or stat (price, %, "
                   "x-faster, size, token count); (b) a direct COMPARISON to a NAMED competitor ('faster than "
                   "GPT-4', 'beats LangChain'); (c) a one-line HOW-IT-WORKS mechanism ('it routes to a smaller "
                   "model first'). A beat that is all opinion/claim with no number, no named comparison, and no "
                   "mechanism is too thin — it forces a generic quote card instead of a designed stat/vs/diagram "
                   "visual. Reach for the concrete fact the brief gives you.\n"
