...
async def test_generation_runner_lands_treated_artifacts_under_the_isolated_job_root(lab, monkeypatch):
    ...
    treatment = stored["clips"][0]["sourceTreatment"]
    assert treatment["schema"] == "content-lab.source-treatment.v1"
    assert treatment["sourceSha256"] == stored["clips"][0]["sha256"]
    assert treatment["generationJobId"] == job_id
    assert treatment["recipeSpecHash"] == recipe_publication()["recipeSpecHash"]
