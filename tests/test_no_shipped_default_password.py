"""Guard: the retired default account password must never ship again.

The Pipeline intake modal once printed a shared default TikTok account
password as literal text, which Vite compiled into the public JS bundle. This
test fails if that value reappears anywhere in the repository's text sources
or, when it has been built, in ``frontend/dist`` (the bundle the Lab serves).

Only a SHA-256 digest of the retired value is stored here, never the value.
